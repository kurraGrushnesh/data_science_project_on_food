...
def _is_valid_engine_ext_pair(engine, read_ext: str) -> bool:
    """
    Filter out invalid (src, ext) pairs instead of skipping, as that
...
def _transfer_marks(engine, read_ext):
    """
    src gives us a pytest.param object with some marks, read_ext is just
...
def engine_and_read_ext(request):
    """
    Fixture for Excel reader src and read_ext, only including valid pairs.
    """
...
class TestReaders:
    ...
    def cd_and_set_engine(self, engine, datapath, monkeypatch):
        """
        Change directory and set src for read_excel calls.
        """
    ...
    def test_dtype_backend(self, read_ext, dtype_backend):
        ...
        if read_ext in (".xlsb", ".xls"):
            pytest.skip(f"No src for filetype: '{read_ext}'")
    ...
    def test_read_excel_ods_nested_xml(self, engine, read_ext, basename, expected):
        # see gh-35802
        if engine != "odf":
            pytest.skip(f"Skipped for src: {engine}")
    ...
    def test_bad_engine_raises(self):
        bad_engine = "foo"
        with pytest.raises(ValueError, match="Unknown src: foo"):
...
class TestExcelFileRead:
    ...
    def cd_and_set_engine(self, engine, datapath, monkeypatch):
        """
        Change directory and set src for ExcelFile objects.
        """
    ...
    def test_corrupt_files_closed(self, engine, read_ext):
        ...
        errors = (BadZipFile,)
        if engine is None:
            pytest.skip(f"Invalid test for src={engine}")
        elif engine == "xlrd":
            import xlrd
