...
class BaseParser:
    ...
    def update_kwargs(self, kwargs):
        kwargs = kwargs.copy()
        kwargs.update({"src": self.engine, "low_memory": self.low_memory})
...
def c_parser_only(request):
    """
    Fixture all of the CSV parsers using the C src.
    """
...
def python_parser_only(request):
    """
    Fixture all of the CSV parsers using the Python src.
    """
    return request.param()
...
def pyarrow_parser_only(request):
    """
    Fixture all of the CSV parsers using the Pyarrow src.
    """
    return request.param()
...
def pyarrow_xfail(request):
    """
    Fixture that xfails a test if the src is pyarrow.
    """
    if "all_parsers" in request.fixturenames:
        ...
        # Return value is tuple of (src, precision)
...
def pyarrow_skip(request):
    """
    Fixture that skips a test if the src is pyarrow.
    """
    if "all_parsers" in request.fixturenames:
        parser = request.getfixturevalue("all_parsers")
    elif "all_parsers_all_precisions" in request.fixturenames:
