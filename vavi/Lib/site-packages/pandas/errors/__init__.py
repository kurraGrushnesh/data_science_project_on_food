...
class ParserWarning(Warning):
    ...
    particular attribute of a CSV file with the requested src.
    ...
    The warning can be avoided by adding `src='python'` as a parameter in
    ...
    ... # ParserWarning: Falling back to the 'python' src...

    Adding `src='python'` to `pd.read_csv` removes the Warning:

    >>> df = pd.read_csv(io.StringIO(csv), sep='[;,]', src='python')
    """
...
class NumbaUtilError(Exception):
    """
    Error raised for unsupported Numba src routines.
    """
...
class NumExprClobberingError(NameError):
    ...
    ``eval`` or ``query`` will throw the error if the src is set
    to 'numexpr'. 'numexpr' is the default src value for these methods if the
    ...
    >>> sin, a = 1, 2
    >>> pd.eval("sin + a", src='numexpr') # doctest: +SKIP
