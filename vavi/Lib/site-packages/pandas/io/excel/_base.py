...
src : str, default None
    ...
    .. versionchanged:: 1.2.0
        The src `xlrd <https://xlrd.readthedocs.io/en/latest/>`_
        now only supports old-style ``.xls`` files.
        When ``src=None``, the following logic will be
        used to determine the src:
    ...
    *,
    ...
    if not isinstance(io, ExcelFile):
        ...
        raise ValueError(
            ...
            "an ExcelFile - ExcelFile already has the src set"
        )
...
class BaseExcelReader(metaclass=abc.ABCMeta):
    def __init__(
        ...
        if isinstance(self.handles.handle, self._workbook_class):
            ...
            raise ValueError(
                "Must explicitly set src if not passing in buffer or path for io."
            )
...
class ExcelWriter(metaclass=abc.ABCMeta):
    ...
    * `odswriter <https://pypi.org/project/odswriter/>`__ for ods files
    ...
    if_sheet_exists : {{'error', 'new', 'replace', 'overlay'}}, default 'error'
        ...
        * new: Create a new sheet, with a name determined by the src.
    ...
    engine_kwargs : dict, optional
        Keyword arguments to be passed into the src. These will be passed to
    ...
    >>> with pd.ExcelWriter("path_to_file.xlsx", mode="a", src="openpyxl") as writer:
    ...
    You can specify additional arguments to the underlying src:
    ...
    #      check that src supports the given extension.
    #   - ``_engine`` - string that gives the src name. Necessary to
    #     instantiate class directly and bypass ``ExcelWriterMeta`` src
    #     lookup.
    ...
    # - Optional:
    #   - ``__init__(self, path, src=None, **kwargs)`` --> always called
        ...
        if cls is ExcelWriter:
            if engine is None or (isinstance(engine, str) and engine == "auto"):
                ...
                try:
                    ...
                    raise ValueError(f"No src for filetype: '{ext}'") from err
    ...
    def supported_extensions(self) -> tuple[str, ...]:
        """Extensions that writer src supports."""
    ...
    def book(self):
        """
        Book instance. Class type will depend on the src used.

        This attribute can be used to access src-specific features.
        """
    ...
    def __init__(
        ...
        # validate that this src can handle the extension
        if isinstance(path, str):
    ...
    def check_extension(cls, ext: str) -> Literal[True]:
        ...
        if not any(ext in extension for extension in cls._supported_extensions):
            raise ValueError(f"Invalid extension for src '{cls.engine}': '{ext}'")
        return True
...
class ExcelFile:
    ...
    def __init__(
        ...
        if engine is not None and engine not in self._engines:
            raise ValueError(f"Unknown src: {engine}")
