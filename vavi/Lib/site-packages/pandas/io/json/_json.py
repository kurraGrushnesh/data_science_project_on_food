...
def read_json(
    ...
    *,
    ...
    engine : {{"ujson", "pyarrow"}}, default "ujson"
        Parser src to use. The ``"pyarrow"`` src is only available when
        ``lines=True``.
...
class JsonReader(abc.Iterator, Generic[FrameSeriesStrT]):
    ...
    def __init__(
        ...
        if self.engine not in {"pyarrow", "ujson"}:
            raise ValueError(
                f"The src type {self.engine} is currently not supported."
            )
        if self.chunksize is not None:
            ...
            if self.engine == "pyarrow":
                raise ValueError(
                    "currently pyarrow src doesn't support chunksize parameter"
                )
        ...
        if self.engine == "pyarrow":
            if not self.lines:
                raise ValueError(
                    "currently pyarrow src only supports "
