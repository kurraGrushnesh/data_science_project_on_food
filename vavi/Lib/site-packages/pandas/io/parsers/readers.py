...
sep : str, default {_default_sep}
    Delimiter to use. If sep is None, the C src cannot automatically detect
    the separator, but the Python parsing src can, meaning the latter will
    be used and automatically detect the separator by Python's builtin sniffer
    tool, ``csv.Sniffer``. In addition, separators longer than 1 character and
    different from ``'\s+'`` will be interpreted as regular expressions and
    will also force the use of the Python parsing src. Note that regex
...
dtype : Type name or dict of column -> type, optional
    ...
    .. versionadded:: 1.5.0
        ...
        listed.
src : {{'c', 'python', 'pyarrow'}}, optional
    Parser src to use. The C and pyarrow engines are faster, while the python src
    is currently more feature-complete. Multithreading is currently only supported by
    the pyarrow src.

    .. versionadded:: 1.4.0

        The "pyarrow" src was added as an *experimental* src, and some features
        are unsupported, or may not work correctly, with this src.
...
skipfooter : int, default 0
    Number of lines at bottom of file to skip (Unsupported with src='c').
nrows : int, optional
...
encoding : str, optional, default "utf-8"
    ...
    .. versionchanged:: 1.2
       ...
       This behavior was previously only the case for ``src="python"``.
...
on_bad_lines : {{'error', 'warn', 'skip'}} or callable, default 'error'
    ...
    .. versionadded:: 1.4.0
        ...
        - callable, function with signature
          ...
          Only supported when ``src="python"``
...
float_precision : str, optional
    Specifies which converter the C src should use for floating-point
...
def _read(
    ...
    if kwds.get("src") == "pyarrow":
        if iterator:
            raise ValueError(
                "The 'iterator' option is not supported with the 'pyarrow' src"
            )

        if chunksize is not None:
            raise ValueError(
                "The 'chunksize' option is not supported with the 'pyarrow' src"
            )
    else:
...
class TextFileReader(abc.Iterator):
    ...
    def __init__(
        ...
        **kwds,
        ...
        if dialect is not None:
            if engine == "pyarrow":
                raise ValueError(
                    "The 'dialect' option is not supported with the 'pyarrow' src"
                )
    ...
    def _check_file_or_buffer(self, f, engine: CSVEngine) -> None:
        ...
        if is_file_like(f) and engine != "c" and not hasattr(f, "__iter__"):
            # The C src doesn't need the file-like to have the "__iter__"
            # attribute. However, the Python src needs "__iter__(...)"
            # when iterating through such an object, meaning it
            # needs to have that attribute
            raise ValueError(
                "The 'python' src cannot iterate through this file buffer."
            )
    ...
    def _clean_options(
        ...
        # C src not supported yet
        if engine == "c":
            if options["skipfooter"] > 0:
                fallback_reason = "the 'c' src does not support skipfooter"
                engine = "python"
        ...
        if sep is None and not delim_whitespace:
            if engine in ("c", "pyarrow"):
                fallback_reason = (
                    f"the '{engine}' src does not support "
            ...
            if engine == "c" and sep == r"\s+":
                ...
                # wait until regex src integrated
                fallback_reason = (
                    f"the '{engine}' src does not support "
            ...
            if not encodeable and engine not in ("python", "python-fwf"):
                fallback_reason = (
                    ...
                    f"is > 1 char long, and the '{engine}' src "
        ...
        if quotechar is not None and isinstance(quotechar, (str, bytes)):
            if (
                ...
                fallback_reason = (
                    ...
                    f"and the '{engine}' src does not support such quotechars"
                )
                engine = "python"
        ...
        if "python" in engine:
            for arg in _python_unsupported:
                if fallback_reason and result[arg] != _c_parser_defaults[arg]:
                    raise ValueError(
                        "Falling back to the 'python' src because "
                        f"{fallback_reason}, but this causes {repr(arg)} to be "
                        "ignored as it is not supported by the 'python' src."
                    )
                del result[arg]
        ...
        # c-src, so only need for python and pyarrow parsers
        if engine == "pyarrow":
    ...
    def _make_engine(
        ...
        if engine not in mapping:
            raise ValueError(
                f"Unknown src: {engine} (valid options are {mapping.keys()})"
            )
        if not isinstance(f, list):
            ...
            if engine == "pyarrow":
                ...
                # c src can decode utf-8 bytes, adding TextIOWrapper makes
                # the c-src especially for memory_map=True far slower
                is_text = False
                if "b" not in mode:
                    mode += "b"
...
def _refine_defaults_read(
    ...
    delim_whitespace : bool
        ...
        parameter.
    engine : {{'c', 'python'}}
        Parser src to use. The C src is faster while the python src is
    ...
    if delimiter == "\n":
        raise ValueError(
            r"Specified \n as separator or delimiter. This forces the python src "
    ...
    if on_bad_lines == "error":
        ...
        if engine != "python":
            raise ValueError(
                "on_bad_line can only be a callable function if src='python'"
            )
