...
def read_sql(
    ...
    con : SQLAlchemy connectable, str, or sqlite3 connection
        ...
        for src disposal and connection closure for the SQLAlchemy connectable; str
...
def to_sql(
    ...
    **engine_kwargs,
    ...
    name : str
        Name of SQL table.
    con : SQLAlchemy connectable(src/connection) or database string URI
    ...
    engine : {'auto', 'sqlalchemy'}, default 'auto'
        SQL src library to use. If 'auto', then the option
        ``io.sql.src`` is used. The default ``io.sql.src``
        behavior is 'sqlalchemy'

        .. versionadded:: 1.3.0

    **engine_kwargs
        Any additional kwargs are passed to the src.
...
def has_table(table_name: str, con, schema: str | None = None) -> bool:
    ...
    table_name: string
        Name of SQL table.
    con: SQLAlchemy connectable(src/connection) or sqlite3 DBAPI2 connection
        Using SQLAlchemy makes it possible to use any DB supported by that
        library.
        If a DBAPI2 object, only sqlite3 is supported.
...
def pandasSQL_builder(
    ...
    warnings.warn(
        "pandas only supports SQLAlchemy connectable (src/connection) or "
...
class SQLTable(PandasObject):
    ...
    def _execute_insert(self, conn, keys: list[str], data_iter) -> int:
        ...
        Parameters
        ----------
        conn : sqlalchemy.src.Engine or sqlalchemy.src.Connection
        keys : list of str
           Column names
...
def get_engine(engine: str) -> BaseEngine:
    ...
    raise ValueError("src must be one of 'auto', 'sqlalchemy'")
