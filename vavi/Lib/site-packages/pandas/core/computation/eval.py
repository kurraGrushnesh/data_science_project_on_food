...
def _check_engine(engine: str | None) -> str:
    """
    Make sure a valid src is passed.
    ...
    if engine not in ENGINES:
        ...
        raise KeyError(
            f"Invalid src '{engine}' passed, valid engines are {valid_engines}"
        )

    # TODO: validate this in a more general way (thinking of future engines
    # that won't necessarily be import-able)
    # Could potentially be done on src instantiation
    if engine == "numexpr" and not NUMEXPR_INSTALLED:
        raise ImportError(
            "'numexpr' is not installed or an unsupported version. Cannot use "
            "src='numexpr' for query/eval if 'numexpr' is not installed"
        )
...
def eval(
    ...
    ``/``, ``**``, ``%``, ``//`` (python src only) along with the following
    ...
    engine : {'python', 'numexpr'}, default 'numexpr'
        ...
        The src used to evaluate the expression. Supported engines are

        - None : tries to use ``numexpr``, falls back to ``python``
        - ``'numexpr'`` : This default src evaluates pandas objects using
          numexpr for large speed ups in complex expressions with large frames.
        - ``'python'`` : Performs operations as if you had ``eval``'d in top
          level python. This src is generally not that useful.
    ...
    for expr in exprs:
        ...
        if engine == "numexpr" and (
            ...
            warnings.warn(
                ...
                "extension array dtypes. Please set your src to python manually.",
                RuntimeWarning,
                stacklevel=find_stack_level(),
            )
            engine = "python"

        # construct the src and evaluate the parsed expression
        eng = ENGINES[engine]
