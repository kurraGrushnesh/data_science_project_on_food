...
f : function, str
    ...
    - String
    - Python function
    - Numba JIT function with ``src='numba'`` specified.

    Only passing a single function is supported with this src.
    If the ``'numba'`` src is chosen, the function must be
...
engine_kwargs : dict, default None
    * For ``'cython'`` src, there are no accepted ``engine_kwargs``
    * For ``'numba'`` src, the src can accept ``nopython``, ``nogil``
      and ``parallel`` dictionary keys. The values must either be ``True`` or
      ``False``. The default ``engine_kwargs`` for the ``'numba'`` src is
...
When using ``src='numba'``, there will be no "fall back" behavior internally.
...
func : function, str, list, dict or None
    ...
    ``src='numba'`` specified. Only passing a single function is supported
    with this src.
...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def _numba_prep(self, data: DataFrame):
        ...
        if len(self.grouper.groupings) > 1:
            raise NotImplementedError(
                "More than 1 grouping labels are not supported with src='numba'"
            )
        # GH 46867
    ...
    def _transform_with_numba(self, func, *args, engine_kwargs=None, **kwargs):
        """
        Perform groupby transform routine with the numba src.
    ...
    def _aggregate_with_numba(self, func, *args, engine_kwargs=None, **kwargs):
        """
        Perform groupby aggregation routine with the numba src.
    ...
    def rolling(self, *args, **kwargs) -> RollingGroupby:
        ...
        method : str {'single', 'table'}, default 'single'
            ...
            This argument is only implemented when specifying ``src='numba'``
            in the method call.
