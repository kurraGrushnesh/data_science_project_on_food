...
class SQLConnection(BaseConnection["Engine"]):
    ...
    - ``url`` or keyword arguments for |sqlalchemy.src.URL.create()|_, except
    ...
    ``sqlalchemy.src.make_url()``. Otherwise, Streamlit requires (at a
    minimum) ``dialect``, ``username``, and ``host``. Streamlit will use
    ``dialect`` and ``driver`` (if defined) to derive ``drivername``, then pass
    the relevant connection parameters to ``sqlalchemy.src.URL.create()``.
    ...
    .. |sqlalchemy.src.URL.create()| replace:: ``sqlalchemy.src.URL.create()``
    .. _sqlalchemy.src.URL.create(): https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.engine.URL.create
    .. |sqlalchemy.src.make_url()| replace:: ``sqlalchemy.src.make_url()``
    .. _sqlalchemy.src.make_url(): https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.engine.make_url
